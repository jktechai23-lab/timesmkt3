/**
 * Versão 30s do "Eu Vi o Futuro Chegar".
 * 60 imagens x 0.5s cada = 30s total. Música trimada nos primeiros 30s.
 */
...
const SIZE = 1080;
const TOTAL_DUR = 30;
const DUR_PER_IMG = TOTAL_DUR / (N_SCENES * 2); // 0.5s
const FRAMES = Math.round(DUR_PER_IMG * FPS);   // 15 frames

fs.mkdirSync(OUT_DIR, { recursive: true });
const outFile = path.join(OUT_DIR, 'eu-vi-o-futuro-chegar-30s.mp4');
...
// com ritmo 2x mais rápido, aumenta levemente o zoomRate pra manter o peso visual
...
filters.push(
  `[${audioIdx}:a]atrim=0:${TOTAL_DUR},asetpts=PTS-STARTPTS,afade=t=out:st=${TOTAL_DUR - 2}:d=2,volume=0.85[aout]`
);
...
console.log(`[video 30s] ${imgs.length} imagens x ${DUR_PER_IMG}s = ${TOTAL_DUR}s @ ${SIZE}x${SIZE} ${FPS}fps`);
const t0 = Date.now();
...
console.log(`\n[video 30s] ✅ ${outFile}`);
console.log(`[video 30s] tamanho: ${(stat.size / 1024 / 1024).toFixed(1)} MB — render em ${wall}s`);
