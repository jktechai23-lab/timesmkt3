/**
 * Renderiza "Mulheres e Tecnologia — 30s" com narração e texto INEMA.CLUB no final.
...
const IMG_DIR = path.join(__dirname, 'prj/inema/outputs/mulheres-tecnologia_2026-04-22/imgs');
const OUT_DIR = path.join(__dirname, 'prj/inema/outputs/mulheres-tecnologia_2026-04-22/video');
const MUSIC = '/tmp/musica_video/mulheres/freesound_719036_Melancholy_Background_Piano__-_bpm_112_l.mp3';
const NARR = '/tmp/narr/narr-mulheres.mp3';
...
const SIZE = 1080;
const TOTAL_DUR = 30;
const DUR_PER_IMG = TOTAL_DUR / (N_SCENES * 2); // 1s
const FRAMES = Math.round(DUR_PER_IMG * FPS);   // 30 frames

fs.mkdirSync(OUT_DIR, { recursive: true });
const outFile = path.join(OUT_DIR, 'mulheres-tecnologia-30s.mp4');
...
// Concat vídeos + drawtext do INEMA.CLUB nos últimos 3s
...
// Áudio: música baixa (narração alta). Narração começa em 2s.
const musicIdx = imgs.length;
const narrIdx = imgs.length + 1;
// Música: 0.35 fora da narração, 0.06 durante. Fade out 2s no final.
// Narração: +1.9x (bem alta), começa em 2s (delay 2000ms)
filters.push(
  `[${musicIdx}:a]atrim=0:${TOTAL_DUR},asetpts=PTS-STARTPTS,volume='if(between(t\\,1.7\\,27)\\,0.06\\,0.35)':eval=frame,afade=t=out:st=${TOTAL_DUR - 2}:d=2[music]`
);
filters.push(`[${narrIdx}:a]adelay=2000|2000,volume=1.9[narr]`);
...
console.log(`[mulheres 30s] ${imgs.length} imagens x ${DUR_PER_IMG}s = ${TOTAL_DUR}s @ ${SIZE}x${SIZE} ${FPS}fps`);
const t0 = Date.now();
...
console.log(`\n[mulheres 30s] ✅ ${outFile}`);
console.log(`[mulheres 30s] tamanho: ${(stat.size / 1024 / 1024).toFixed(1)} MB — render em ${wall}s`);
